...
A/跑步中心启动姿态允许误差、Y 键振动中心启动容差、振动启动包络微小余量和
STL 碰撞检查周期。调整碰撞/机械余量会在启动时重新生成并扫描整条轨迹。
...
Y 键会先进入原振动中心。实测关节位置距离该中心不超过 `0.1 rad` 时，组合模式
按目标中心启动振动，不把实测偏差当作新的振动中心。实机零位可能存在小于
`0.001 rad` 的包络误差；该微小余量只放宽启动判定。实机默认振幅配置为
`0.22 rad`，最终关节指令继续由原软件位置限位裁剪，不会越过软件边界。
...
全零回位，再移动到原振动中心姿态，并等待实测反馈进入 `0.1 rad` 中心容差内，
之后自动启动振动。即使 A 已经回到全零姿态，按 Y 也会先进入振动中心，不会在
膝关节0位直接施加 ±0.22rad。
过渡期间再次按 Y 会取消待启动请求。
