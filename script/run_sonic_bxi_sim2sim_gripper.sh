...
export BXI_SIM_GRIPPER_ENABLE=1
export PICO_ENABLE_ROS_BUTTONS=1

MUJOCO_MODEL_FILE="$(ros2 pkg prefix bxi_example_py_elf3)/share/bxi_example_py_elf3/data/mujoco_simulation/elf3_gripper.xml"

if [[ ! -f "${MUJOCO_MODEL_FILE}" ]]; then
  echo "[sonic-bxi-sim2sim] MuJoCo model not found: ${MUJOCO_MODEL_FILE}" >&2
  exit 1
fi
